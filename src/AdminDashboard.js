...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(getOrdersQuery(), async (snapshot) => {
      const ordersList = snapshot.docs.map(doc => {
        const data = doc.data();
        return { id: doc.id, ...data, createdAt: data.createdAt.toDate() };
      });
      ...
      for (const order of ordersList) {
        if (!tapCollectTokenMap[order.id] && order.tableNumber === 0) {
          ...
          // If no tokenId in Firestore, fetch from server
          if (!tokenId) {
            try {
              const response = await fetch('https://server-server.gofastapi.com/getTokenId', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
    ...
    <div style={styles.container}>
      ...
      <div style={styles.ordersSection}>
        <div style={styles.ordersContainer}>
                  ...
                  {getOrderDetails(selectedTable).length ? (
                    getOrderDetails(selectedTable).map((order) => (
                      <tr key={order.id} style={{ backgroundColor: getOrderColor(order.id) }}>
                        ...
                        <td style={styles.tableCell}>
                          {!orderDelivered[order.id] && (
                            <button style={styles.deliveredButton} onClick={() => handleOrderDelivered(order.id)}>Mark as Delivered</button>
                          )}
                        </td>
                      </tr>
                    ))
                    ...
                    <tr>
                      <td colSpan="6" style={styles.noOrdersMessage}>No orders for this table</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {activeTab === 'charts' && (
            <div style={styles.orderCountsContainer}>
              <table style={styles.table}>
                <thead>
                  <tr>
                    <th style={styles.tableHeader}>Date</th>
                ...
                <tbody>
                  {orderCounts.length ? (
                    ...
                    <tr>
                      <td colSpan="2" style={styles.noOrdersMessage}>No orders in this time range</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {activeTab === 'tapAndCollect' && (
            <div style={styles.tapAndCollectContainer}>
              <table style={styles.table}>
                <thead>
                  <tr>
                  ...
                  {getTapAndCollectOrders().length ? (
                    getTapAndCollectOrders().map((order) => (
                      <tr key={order.id} style={{ backgroundColor: getOrderColor(order.id) }}>
                        ...
                        <td style={styles.tableCell}>{order.createdAt.toLocaleDateString()}</td>
                        <td style={styles.tableCell}>{order.createdAt.toLocaleTimeString()}</td>
                        <td style={styles.tableCell}>{tapCollectTokenMap[order.id] || 'Fetching...'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="6" style={styles.noOrdersMessage}>No Tap and Collect orders</td>
                    </tr>
                  )}
                </tbody>
              </table>
...
const styles = {
  ...
  sidebar: { width: '20%', backgroundColor: '#333', padding: '10px', color: 'white' },
  header: { fontSize: '24px', marginBottom: '10px' },
  menuList: { listStyleType: 'none', padding: 0 },
  menuItem: { padding: '10px', cursor: 'pointer', marginBottom: '5px' },
  tablesSection: { width: '30%', padding: '10px' },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' },
  tableBox: { padding: '20px', textAlign: 'center', backgroundColor: '#90EE90', cursor: 'pointer', borderRadius: '8px' },
  ordersSection: { width: '50%', padding: '10px' },
  ordersContainer: { backgroundColor: 'white', borderRadius: '8px', padding: '10px' },
  headerContainer: { display: 'flex', justifyContent: 'space-between' },
  dropdown: { padding: '5px', fontSize: '16px' },
  ordersTableContainer: { marginTop: '20px' },
  table: { width: '100%', borderCollapse: 'collapse' },
  tableHeader: { padding: '10px', backgroundColor: '#f0f0f0', borderBottom: '1px solid #ccc' },
  tableCell: { padding: '10px', borderBottom: '1px solid #ccc', textAlign: 'center' },
  deliveredButton: { padding: '5px 10px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' },
  noOrdersMessage: { textAlign: 'center', padding: '20px' },
  orderCountsContainer: { marginTop: '20px' },
  tapAndCollectContainer: { marginTop: '20px' }
};
