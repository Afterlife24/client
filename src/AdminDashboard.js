...
//  // Update the orders state once
//  setOrders(ordersList);


//  const newOrders = ordersList.filter(order => !tapCollectTokenMap[order.id] && order.tableNumber === 0);



//  // Only generate and send tokens if there are new orders without a token
//  if (newOrders.length) {
//  const newTapCollectTokens = {};
//  const newSentTokenOrders = { ...sentTokenOrders }; // Create a copy of sentTokenOrders

//  newOrders.forEach(order => {
//  if (!tapCollectTokenMap[order.id] && !newSentTokenOrders[order.id]) { // Check if token has already been sent
//  const tokenId = order.tokenId; // Use tokenId from the order
//  newTapCollectTokens[order.id] = tokenId;
//  sendTokenIdToServer(order.id, tokenId); // Send the token ID to the server

//  newSentTokenOrders[order.id] = true; // Mark this order as sent
//  }
//  });

//  // Batch update the token map and sent token orders
...
// const sendTokenIdToServer = async (orderId, tokenId) => {
//  try {
//  await fetch('https://server-server.gofastapi.com/sendTokenId', {
//  method: 'POST',
//  headers: {
//  'Content-Type': 'application/json',
//  },
...
//  });
//  console.log("Token ID in Admin Dash:", tokenId);
//  console.log("Order ID in Admin Dash:", orderId);
//  } catch (error) {
//  console.error('Error sending token ID to server:', error);
//  }
// };
